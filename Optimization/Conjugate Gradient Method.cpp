...
int main()
{
    ...
    fstream fout("Output_Conjugate_Gradient_Method.txt", ios :: out);
    fstream fo("Output_Conjugate_Gradient_Method.xls", ios :: out);
    fo<<"Iteration No."<<"\t"<<"X1"<<"\t"<<"X2"<<"\t"<<"f(X1,X2)"<<"\n\n";
    double x1_initial, x2_initial, epsilon, x1, x2, counter=0, gradient[20], d[20], mod=1000, delta, alpha[3], interval, delta1, delta2, alpha_a, alpha_b, gss_epsilon, alpha_final, beta;
    ...
    while (mod >= epsilon)
    {
          ...
          beta = ((gradient[0] * gradient[0]) + (gradient[1] * gradient[1])) / (mod*mod);
          d[0] = -gradient[0] + (beta * d[0]);
          d[1] = -gradient[1] + (beta * d[1]);   
    }
    return 1;
}      
